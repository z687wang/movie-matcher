...
class ActorDetailsViewController: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    override func viewDidLoad() {
        ...
        if actorItem.fullyDetailed {
            ...
            apiClient.fetchActorDetails(actor: actorItem) { [weak self] in
                self?.isContentReady = true
            }
        }
...
//        navigationController?.navigationBar.barStyle = .default
    ...
    func insertGradientBackground() {
        ...
        self.view.backgroundColor = self.botColor
//        self.view.layer.insertSublayer(self.gradientLayer!, at:0)
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if let cell = cell as? MediaDescriptionCellView {
            ...
            if let imageUrl = item.posterURL {
                Nuke.loadImage(
                    with: imageUrl,
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        if indexPath.section == 1 {
            let movie = actorItem.relatedMovies[indexPath.item]
            let destVC = UIStoryboard(name: "Main", bundle: nil).instantiateViewController(withIdentifier: "MyMovieDetailViewController") as! MovieDetailViewController
            destVC.movieData = movie
